...
public class BasicLearner {
	...
	public static EquivalenceOracle<MealyMachine<?, String, ?, String>, String, Word<String>> loadTester(
		...
		switch (testMethod){
			...
			case RandomWalk:
				return new RandomWalkEQOracle<String, String>(sul, randomWalk_chanceOfResetting, randomWalk_numberOfSymbols, true, new Random(123456l));
			// Other methods are somewhat smarter than random testing: state coverage, trying to distinguish states, etc.
			case WMethod:
				return new WMethodEQOracle.MealyWMethodEQOracle<String, String>(sulOracle, w_wp_methods_maxDepth);
			case WpMethod:
				return new WpMethodEQOracle.MealyWpMethodEQOracle<String, String>(sulOracle, w_wp_methods_maxDepth);
			case UserQueries:
...
System.out.println("Looking for counter example");
				// Search for CE
